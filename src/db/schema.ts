...
export const activityTable = pgTable(
  "activities",
  {
    ...
    begintime: varchar("begintime").notNull(),
    endtime: varchar("endtime").notNull(),
  },
  (table) => ({
    userHandleIndex: index("user_handle_index").on(table.userHandle),
    // begintimeIndex: index("begintime_index").on(table.begintime),
...
export const timeTable = pgTable(
  ...
  (table) => ({
    userHandleIndex: index("user_handle_index").on(table.userHandle),
    dayIndex: index("day_index").on(table.day),
    hourIndex: index("hour_index").on(table.hour),
  }),
);
